...
export class FestivaljsonService {
  ...
  getFestival(id: any): Observable<any> {
    return this.http.get<Festival[]>('assets/festivals.json').pipe(
      map((data) => {
        return data.find((festival) => festival.id === id);
      })
    );
  }

  /*   getFestival(id: number): Observable<any> {
    return this.http.get('assets/festivals.json').pipe(
      map((data) => {
        return data.find((festival: { id: number }) => festival.id === id);
      })
    );
  } */
}
